     ...
     */
					...
					dayPicker: {
						...
						onSelect: function onSelect(selectedDayUnix) {
							Helper.debug(
								this,
								"dayPicker Event: onSelect : " + selectedDayUnix
							);
						},
					},
					...
					yearPicker: {
						...
						titleFormatter: function titleFormatter(year) {
							...
							return (
								startYear.format(this.model.options.yearPicker.titleFormat) +
								"-" +
								endYear.format(this.model.options.yearPicker.titleFormat)
							);
						},
					...
					_createClass(Input, [
						...
						},
						{
							...
							value: function parseInput(inputString) {
								...
								if (parse.parse(inputString) !== undefined) {
									var pd = this.model.PersianDate.date(
										parse.parse(inputString)
									).valueOf();
							...
							value: function observe() {
								...
								$(that.elem).bind("keyup", function (e) {
									...
									if (
										...
										(ctrlDown &&
											(e.keyCode == vKey || $.inArray(e.keyCode, ctrlKey) > 0))
									) {
										trueKey = true;
									}
									if (trueKey) {
						...
						},
						{
							...
							value: function _attachInputElementEvents() {
								...
								var closePickerHandler = function closePickerHandler(e) {
									if (
										!$(e.target).is(that.elem) &&
										!$(e.target).is(that.model.view.$container) &&
										$(e.target).closest(
											"#" + that.model.view.$container.attr("id")
										).length == 0 &&
								...
								$(this.elem).on(
									...
									Helper.debounce(function (evt) {
										...
										if (that.model.state.ui.isInline === false) {
											$("body")
												.unbind("click", closePickerHandler)
												.bind("click", closePickerHandler);
										}
										if (Helper.isMobile) {
											$(this).blur();
										}
						...
						},
						{
							...
							value: function update(unix) {
								if (
									this.model.options.initialValue == false &&
									this._firstUpdate
								) {
						...
						},
						{
							...
							value: function getOnInitState() {
								var persianDatePickerTimeRegex =
								...
								if (
									inputValue &&
									inputValue.match(persianDatePickerTimeRegex)
								) {
									...
									if (
										this.model.options.initialValueType === "persian" &&
										inputValue
									) {
										var parse = new PersianDateParser();
										...
										this.model.options.initialValueType === "unix" &&
										inputValue
									) {
					...
					_createClass(Navigator, [
						{
							...
							value: function liveAttach() {
								...
								if (this.model.options.navigator.scroll.enabled) {
									var that = this;
									var gridPlot = $(
										"#" + that.model.view.id + " .datepicker-grid-view"
									)[0];
									...
									if (this.model.options.timePicker.enabled) {
										$("#" + that.model.view.id + " .time-segment").each(
											function () {
												Hamster(this).wheel(function (event, delta) {
													var $target = $(event.target);
													var key = $target.data("time-key")
														? $target.data("time-key")
														: $target
								...
								if (this.model.options.dayPicker.enabled) {
									...
									$(document).on(
										...
										function () {
											...
											if (
												that.model.state.selected.month !==
												that.model.state.view.month
											) {
												mustRender = true;
											} else {
												mustRender = false;
											}
											that.model.state.setViewDateTime(
												"unix",
												that.model.state.selected.unixDate
											);
					...
					_createClass(Options, [
						{
							...
							value: function _compatibility(options) {
								...
								persianDate.toLocale(
									options.calendar[options.calendarType].locale
								);
			...
			/***/ function (module, exports, __webpack_require__) {
				...
				var State = (function () {
					...
					function State(model) {
						 ...
						 */
						this.viewMode =
							this.viewModeList.indexOf(model.options.viewMode) > 0
								? model.options.viewMode
								: this.viewModeList[0];
						...
						this.viewModeIndex =
							this.viewModeList.indexOf(model.options.viewMode) > 0
								? this.viewModeList.indexOf(model.options.viewMode)
								: 0; // defaul 'day'
					...
					_createClass(State, [
						...
						},
						{
							...
							value: function _setViewDateTimeUnix() {
								var daysInMonth = new persianDate().daysInMonth(
									this.view.year,
									this.view.month
								);
					...
					_createClass(Toolbox, [
						...
						},
						{
							...
							value: function _attachEvents() {
								...
								$(document).on(
									"click",
									"#" + that.model.view.id + " .pwt-btn-today",
									function () {
										that.model.state.setSelectedDateTime(
											"unix",
											new Date().valueOf()
										);
										that.model.state.setViewDateTime(
								...
								$(document).on(
									"click",
									"#" + that.model.view.id + " .pwt-btn-calendar",
									function () {
										...
										that.model.state.setViewDateTime(
											"unix",
											that.model.state.view.unixDate
										);
										that.model.view.render();
								...
								$(document).on(
									"click",
									"#" + that.model.view.id + " .pwt-btn-submit",
									function () {
										that.model.view.hide();
			...
			/***/ function (module, exports, __webpack_require__) {
				...
				var View = (function () {
					...
					function View(model) {
						 ...
						 */
						this.id =
							"persianDateInstance-" + parseInt(Math.random(100) * 1000);
						var that = this;
					...
					_createClass(View, [
						...
						},
						{
							...
							value: function setPickerBoxPosition() {
								...
								if (this.model.options.position === "auto") {
									...
									this.$container.css({
										left:
											this.model.options.position[1] +
											inputPosition.left +
											"px",
										top:
											this.model.options.position[0] + inputPosition.top + "px",
									});
								}
							},
						...
						},
						{
							...
							value: function _getYearViewModel(viewState) {
								 ...
								 */
								var list = []
									.concat(_toConsumableArray(Array(this.yearsViewCount).keys()))
									.map(function (value) {
										return (
											value +
											parseInt(viewState.year / _this.yearsViewCount) *
												_this.yearsViewCount
										);
									});
								/*
						...
						},
						{
							...
							value: function _getMonthViewModel() {
								...
								try {
									for (
										var _iterator2 = that.model.PersianDate.date()
												.rangeName()
												.months.entries()
												[Symbol.iterator](),
											_step2;
							...
							value: function checkDayAccess(unixtimespan) {
								...
								if (self.model.state.filetredDate) {
									if (self.minDate && self.maxDate) {
										...
										if (
											!(
												unixtimespan >= self.minDate &&
												unixtimespan <= self.maxDate
											)
										) {
											return false;
										}
						...
						},
						{
							...
							value: function _getDayViewModel() {
								...
								var pdateInstance = this.model.PersianDate.date(),
									...
									firstWeekDayOfMonth =
										pdateInstance.getFirstWeekDayOfMonth(viewYear, viewMonth) -
										1,
									outputList = [],
									daysListindex = 0,
								...
								try {
									for (
										var _iterator3 = daysMatrix.entries()[Symbol.iterator](),
											_step3;
										...
										try {
											for (
												var _iterator4 = daysRow.entries()[Symbol.iterator](),
													_step4;
						...
						},
						{
							...
							value: function markSelectedDay() {
								...
								this.$container.find(".table-days td").each(function () {
									if (
										$(this).data("date") ==
										[selected.year, selected.month, selected.date].join(",")
									) {
						...
						},
						{
							...
							value: function getViewModel(data) {
								...
								return {
									...
									altCalendarShowHint: this.model.options.calendar[
										anotherCalendar[0]
									].showHint,
						...
						},
						{
							...
							value: function renderTimePartial() {
								var timeViewModel = this._getTimeViewModel(
				...
				(function (window, document) {
					...
					Hamster.Instance.prototype = {
						...
						unwheel: function offEvent(handler, useCapture) {
							...
							if (
								handler === undefined &&
								(handler = this.handlers.slice(-1)[0])
							) {
					...
					Hamster.event = {
						...
						add: function add(hamster, eventName, handler, useCapture) {
							...
							hamster.element[Hamster.ADD_EVENT](
								Hamster.PREFIX + eventName,
								handler,
								useCapture || false
							);
						...
						remove: function remove(hamster, eventName, handler, useCapture) {
							...
							hamster.element[Hamster.REMOVE_EVENT](
								Hamster.PREFIX + eventName,
								handler,
								useCapture || false
							);
					...
					function parseTemplate(template, tags) {
						...
						function compileTags(tagsToCompile) {
							...
							openingTagRe = new RegExp(
								escapeRegExp(tagsToCompile[0]) + "\\s*"
							);
